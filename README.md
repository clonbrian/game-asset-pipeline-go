...
- Batch API 建立作業不是 idempotent；重複送同一批會建立新作業。專案會把 metadata 落在 `outputDir/gemini_batch_job_meta.json`，在 `overwrite=false` 下優先重用既有 job。
...
- **`modelPreset`**（可選）：預設 **`gemini_default_realtime`**。切換成 `gemini_default_batch` 或 `imagen_fast_test` 即可換路線，不需手動改多個欄位。
